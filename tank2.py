        ...
        for barrier in self.scene.barriers:
            ...
            if self.collidesWith(barrier):
                if dir == "right" and self.isKeyPressed(pygame.K_w):
                    if self.right > barrier.left and self.top != barrier.bottom:
                        self.right = barrier.left  
                        self.speed = 0
                if dir == "right" and self.isKeyPressed(pygame.K_s):
                    if self.left < barrier.right and self.top != barrier.bottom:
                        self.left = barrier.right  
                        self.speed = 0
                if dir == "left" and self.isKeyPressed(pygame.K_w):
                    if self.left < barrier.right and self.bottom != barrier.top:
                        self.left = barrier.right 
                        self.speed = 0    
                if dir == "left" and self.isKeyPressed(pygame.K_s):
                    if self.right > barrier.left and self.bottom != barrier.top:
                        self.right = barrier.left 
                        self.speed = 0    
                if dir == "down" and self.isKeyPressed(pygame.K_w):
                    if self.bottom > barrier.top and self.left != barrier.right:
                        self.bottom = barrier.top 
                        self.speed = 0
                if dir == "down" and self.isKeyPressed(pygame.K_s):
                    if self.top < barrier.bottom and self.left != barrier.right:
                        self.top = barrier.bottom 
                        self.speed = 0
                if dir == "up" and self.isKeyPressed(pygame.K_w):
                    if self.top < barrier.bottom and self.right != barrier.left:
                        self.top = barrier.bottom 
                        self.speed = 0
                if dir == "up" and self.isKeyPressed(pygame.K_s):
                    if self.bottom > barrier.top and self.right != barrier.left:
                        self.bottom = barrier.top 
                        self.speed = 0
